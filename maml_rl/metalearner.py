...
class MetaLearner(object):
    ...
    def adapt(self, episodes, first_order=False, toprint=False):
        """Adapt the parameters of the policy network to a new task, from 
        sampled trajectories `episodes`, with a one-step gradient update [1].
        """
        # Fit the baseline to the training episodes

        criticloss = self.baseline.fit(episodes, isnew=True)
        oldstate = self.policy.state_dict()
        actorloss = self.policy.fit(episodes, self.baseline, isnew=True)

        if toprint:
            print("Avg-Critic Loss: ", criticloss,
                  "  Avg-Actor Loss: ", actorloss)

        # Get the loss on the training episodes
        # opt = torch.optim.RMSprop(self.policy.parameters(), lr=self.fast_lr, alpha=0.9)
        # # old_params = OrderedDict(self.policy.named_parameters())
        # for itr in range(1):
        ...
        #     #     print("same-------")
        params = OrderedDict(self.policy.named_parameters())
        self.policy.load_state_dict(oldstate)

        if params == self.policy.parameters():
            print("same~~~~~~~")
    ...
    def sample(self, tasks, first_order=False):
        ...
        for task in tasks:
            tempid += 1
            print("Sampling task ", tempid, " in ", len(tasks))

            self.sampler.reset_task(task)
            train_episodes = self.sampler.sample(self.policy,
                gamma=self.gamma, device=self.device)

            params = self.adapt(train_episodes, first_order=first_order, toprint=True)
    ...
    def step(self, episodes, max_kl=1e-3, cg_iters=10, cg_damping=1e-2,
        ...
        for _ in range(ls_max_steps):
            ...
            if self.policy.paramsFlag == OrderedDict(self.policy.named_parameters()):
                print("really same~~~~~~~~")
