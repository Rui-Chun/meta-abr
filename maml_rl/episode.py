...
class BatchEpisodes(object):
    def __init__(self, batch_size, gamma=0.95, device='cpu'):
        ...
        self._entropy_list = [[] for _ in range(batch_size)]
        self._mask_list = []
    ...
    @property
    def entropys(self):
        if self._entropys is None:
            entropys = np.zeros((len(self), self.batch_size), dtype=np.float32)
            for i in range(self.batch_size):
                length = len(self._entropy_list[i])
                entropys[:length, i] = np.stack(self._entropy_list[i], axis=0)
            self._entropys = torch.from_numpy(entropys).to(self.device)
        return self._entropys
    ...
    def append(self, s_batch, a_batch, r_batch, info, batch_id):
        self._observations_list[batch_id] = s_batch
        self._actions_list[batch_id] = a_batch
        self._rewards_list[batch_id] = r_batch
        self._entropy_list[batch_id] = info['entropy']
